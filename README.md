...
> Predictive-Analytics - Open source tool by AetherCodeHQ

`ai` `analytics` `cli` `golang` `prediction`

## What is it?

**Predictive Analytics** is a ai/ml tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
go build -o Predictive-Analytics .
```
...
```bash
# Basic usage
./Predictive-Analytics --path ./target

# With options
./Predictive-Analytics --path ./target --format json --output report.json

# Verbose mode
./Predictive-Analytics --path ./target --verbose
```
...
### Basic scan
```bash
./Predictive-Analytics --path ./my-project
```

### JSON report
```bash
./Predictive-Analytics --path ./my-project --format json --output report.json
```
...
- name: Run Predictive Analytics
  run: ./Predictive-Analytics --path . --format json --output report.json
```
